...
public class Database extends SQLiteOpenHelper {
  ...
  //  private final String ALARM_VIBRATE="alarm_vibrate";
    ...
    private final String ALARM_SNOOZE_ACTIVE="alarm_snooze_active";
  ...
  //  private static String ;
    ...
    private Database(Context context)
    {
        ...
        if(checkifDBExists()) {
            alarmList=getAlarmsFromDataBase();
            initRingTones();
            return;
        }
    ...
    private boolean checkifDBExists()
    {
        ...
        if(myDatabase==null) {
            try {
                ...
                //   Toast.makeText(context,"DB Exists",Toasta.LENGTH_LONG).show();
    ...
    public void onCreate(SQLiteDatabase database)
    {
        database.execSQL("CREATE TABLE IF NOT EXISTS "+ ALARM_TABLE +
                ...
                + ALARM_SNOOZE_ACTIVE + " INTEGER NOT NULL,"
  ...
  /*  public List<Alarm> getAlarms()
    ...
    {
        String [] columns= new String[]{ALARM_ID,ALARM_NAME,ALARM_TIME,ALARM_TONE,ALARM_ACTIVE,ALARM_DAYS,ALARM_SNOOZE,ALARM_SNOOZE_ACTIVE};
       ...
       // Toast.makeText(context,String.valueOf(cursor.getCount()),Toast.LENGTH_LONG).show();
        ...
        if(cursor.moveToFirst())
        {
            ...
            {
                alarms[i]=new Alarm();
                alarms[i].setPrimaryKeyDB(cursor.getInt(cursor.getColumnIndex(ALARM_ID)));
                ...
                {
                    alarms[i].setSnoozeFeature(true);
                }
                else
                {
                    alarms[i].setSnoozeFeature(false);
                }

                if(cursor.getInt(cursor.getColumnIndex(ALARM_SNOOZE_ACTIVE))==1) {
                    alarms[i].setActiveSnooze(true);
                }
                else {
                    alarms[i].setActiveSnooze(false);
                }
    ...
    public void updateAlarm(Alarm alarm)
    {
        ...
        cv.put(ALARM_SNOOZE_ACTIVE,alarm.isSnoozing() ? 1 : 0);
        ...
        try {
            ...
            myDatabase.update(ALARM_TABLE,cv,"alarm_id=?",new String[]{Integer.toString(alarm.getPrimaryKeyDB())});
    ...
    {
        try {
            myDatabase.delete(ALARM_TABLE, "alarm_id=?", new String[]{String.valueOf(alarm.getPrimaryKeyDB())});
